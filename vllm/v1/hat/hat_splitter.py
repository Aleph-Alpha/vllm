...
class HATRuleSplitter(HATSplitter):
    ...
    def __init__(self, special_token_dict: dict | None = None, max_word_size: int = 100):
        super().__init__()
        ...
        for str_chunk in self.special_token_pattern.split(text):
            if str_chunk:
                if str_chunk in self.special_token_dict:
                    ...
                    chunks.extend(
                        list(chunk) for chunk in self.hat_splitter.split_with_limit(str_chunk, self.max_word_size)
                    )
        return chunks
