...
class HATFixedSplitter(HATSplitter):

    def __init__(self,
                 special_token_dict: dict | None = None):
        super().__init__()
        print(f"###############HAT_FIXED_SIZE_SPLITTER_CHUNK: {envs.HAT_FIXED_SIZE_SPLITTER_CHUNK}")
        self.chunk_size = int(envs.HAT_FIXED_SIZE_SPLITTER_CHUNK)
        self.max_word_size = int(envs.HAT_FIXED_SIZE_SPLITTER_CHUNK)
        self.special_token_dict = special_token_dict or {}
        self.special_token_replace: dict[int, list[int]] = {
            token: list(text.encode("utf-8"))
            for text, token in self.special_token_dict.items()
        }
        self.special_token_pattern = (re.compile(
            rf"({'|'.join(map(re.escape, self.special_token_dict.keys()))})")
                                      if self.special_token_dict else
                                      re.compile(r"(?!)"))
        self.eot_id: int = self.special_token_dict.get('<|eot_id|>', 192)
        ...
        for str_chunk in self.special_token_pattern.split(text):
            if str_chunk:
                if str_chunk in self.special_token_dict:
                    ...
                    # Convert to bytes and split into fixed-size chunks
                    byte_data = list(str_chunk.encode("utf-8"))
                    for i in range(0, len(byte_data), self.chunk_size):
                        chunks.append(byte_data[i:i + self.chunk_size])
        return chunks
