...
from vllm.v1.hat.hat_utils import HATRequestType, HATSequenceState, HATEncoderConnectorInput, HATEncoderHiddenStatesPhases, _create_empty_model_runner_output, _create_empty_scheduler_output, check_byte_for_new_word, safe_list_slice, safe_tensor_slice, split_text
...
class HATManager:
    ...
    def add_request(self, scheduler_output: SchedulerOutput) -> Tuple[SchedulerOutput, SchedulerOutput]:
        ...
        for cached_req_data in scheduler_output.scheduled_cached_reqs:
            ...
            match (req_state.request_type, cached_req_data.resumed_from_preemption):
                case (HATRequestType.CHUNKED_PREFILL, _) | (_, True):
                    ...
                    cached_req_data_backbone = CachedRequestData(
                        ...
                        num_computed_tokens=self.req_ids_to_hat_state[req_id].word_position_cpu,
                    )
                ...
                case (HATRequestType.DECODE_WORD_BOUNDARY, _):
                    ...
                    if len(req_state.block_table_backbone) > 0:
                        for idx in range(len(block_table_backbone)):
                            req_state.block_table_backbone[idx].extend(block_table_backbone[idx])
                    else:
                    ...
                    cached_req_data_backbone = CachedRequestData(
                        ...
                        resumed_from_preemption=cached_req_data.resumed_from_preemption,
                        new_token_ids=[0],
                        new_block_ids=req_state.block_table_backbone,
                        num_computed_tokens=self.req_ids_to_hat_state[req_id].word_position_cpu,
                    )
                ...
                case _:
                    raise ValueError(f"Invalid request type: {req_state.request_type}")
    ...
    def handle_encoder_output(self, scheduler_output_byte: SchedulerOutput,
                              encoder_hidden_states: torch.Tensor) -> Tuple[HATEncoderHiddenStatesPhases,
                                                                            SchedulerOutput,
                                                                            SchedulerOutput]:
    ...
    def prepare_input_encoder_connector(self, scheduler_output_word: SchedulerOutput,
                                        encoder_hidden_states_encoder_connector: Optional[List[torch.Tensor]] = None) -> HATEncoderConnectorInput:
    ...
    def update_backbone_info_decode_path(self, predictive_word_embeddings: torch.Tensor):
        predictive_word_embeddings_copy = predictive_word_embeddings.clone()
        for i, scheduled_req in enumerate(self.scheduler_output_word_decodes.scheduled_cached_reqs):
            req_id = scheduled_req.req_id
            req_state = self.req_ids_to_hat_state[req_id]
            ...
            req_state.prev_pred_backbone_embedding = predictive_word_embeddings_copy[i:i+1, :]
            req_state.request_type = HATRequestType.DECODE
            
    def update_backbone_info_prefill_path(self, scheduler_output_word: SchedulerOutput):
    ...
    def process_outputs_prefill_chunked_prefill(self, scheduler_output_byte_final_decoder: SchedulerOutput, model_runner_output: ModelRunnerOutput):
        ...
        for scheduled_req in scheduler_output_byte_final_decoder.scheduled_new_reqs + cached_reqs:
            ...
            new_token_id = None
            last_chunked_prefill_condition = req_state.num_prompt_tokens == (req_state.num_scheduled_tokens_byte + scheduled_req.num_computed_tokens)
    ...
    def process_outputs_enc_dec_loop(self, scheduled_cached_reqs: List[CachedRequestData], model_runner_output: ModelRunnerOutput, decode_path: bool = True) -> SchedulerOutput:
        """
        ...
        for scheduled_req in scheduled_cached_reqs:
            ...
            if is_new_word:
                ...
                if decode_path:
                    ...
                    self.scheduler_output_word_decodes.num_scheduled_tokens[req_id] = 1
                    self.scheduler_output_word_decodes.total_num_scheduled_tokens += 1
                    
                    self.scheduler_output_word_decodes.scheduled_cached_reqs.append(cached_req_data_backbone)
